...
**このルールが決めるのはソースコードだけ。** 画像やフォントの置き場所は[静的ファイルの置き場所](#静的ファイルの置き場所)で別に決める。
...
新しいファイルを作るとき、上から順に当てはめる。かけるのはソースコードだけで、画像などの静的ファイルはこの流れに乗せない（[静的ファイルの置き場所](#静的ファイルの置き場所)）。
...
画像・フォント・アイコンなどの静的ファイルは、まとまりでは決めない。Vite の作法で決める。

| 条件 | 置き場所 |
| --- | --- |
| ソースから参照する | 使う場所の隣に置いて import する |
| ソースから参照しない（`robots.txt` など） | `public/`（`src/` の外、アプリ直下） |
| ファイル名をそのまま保つ必要がある | 同上 |

**既定は import。** import したものはビルドで内容ハッシュの付いた名前になるので、中身が変われば URL も変わり、長期キャッシュを効かせられる。パスを間違えればビルドが落ち、参照されなくなったファイルは出力から外れる。`public/` に置いたものはこのどれも起きない。名前は据え置き、綴り間違いは実行時の 404、使わなくなっても残り続ける。

だから `public/` を選ぶのは、**URL が固定でないと困るとき**に限る。`robots.txt`、`sitemap.xml`、meta タグから参照する OG 画像、外部から直リンクされるファイル。

import する静的ファイルは、使う場所の隣に置く。

```text
src/screens/user-list/components/users/
├─ index.tsx
├─ user-card.tsx
└─ avatar-placeholder.webp
```

**これは「まとまりだから隣に置く」のではない。** import するかぎり置き場所は出力に影響しないので、いちばん近いところに置いている。結果としてまとまりごと動かせる。
...
| `apps/app/public/`（`src/` の外） | ソースから参照しない静的ファイルが要るようになったとき（[静的ファイルの置き場所](#静的ファイルの置き場所)） |
